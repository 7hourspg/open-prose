...
export const Route = createFileRoute("/projects/$projectId/about")({
...
function AboutRoute() {
  ...
  useEffect(() => {
    if (project.data && !hydrated) {
      setContent(project.data.aboutContent ?? "");
      setHydrated(true);
    }
  ...
  const scheduleSave = (next: string) => {
    ...
    timer.current = window.setTimeout(async () => {
      const updated = { ...project.data!, aboutContent: next } as domain.Project;
      try {
  ...
  const onChange = (next: string) => {
    setContent(next);
    if ((project.data?.aboutContent ?? "") === next) {
      if (timer.current) window.clearTimeout(timer.current);
      return;
    }
    scheduleSave(next);
  ...
  return (
    <EditorPane
      crumbTrail={
        <>
          ...
          <CrumbSep />
          <CrumbCurrent>The about page</CrumbCurrent>
        </>
      }
      status={
        <>
    ...
    >
      <MarkdownEditor
        ...
        placeholder="A few words about you, or whatever this site is."
        header={<AboutHeader />}
      />
    </EditorPane>
  );
